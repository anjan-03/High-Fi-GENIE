...
#define ANCHOR_ADD "82:17:5B:D5:A9:9A:E2:9C" //for anchor A2
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len) {
  ...
  receivedData.count++;
  Serial.println(receivedData.count);
  if ((char)receivedData.path[receivedData.count]=='C'){
      esp_err_t result1 = esp_now_send(broadcastAddress3, (uint8_t *) &receivedData, sizeof(receivedData));
      if (result1==ESP_OK){
  ...
  else if (receivedData.path[receivedData.count]=='B'){
